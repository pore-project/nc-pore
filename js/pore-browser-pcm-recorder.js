...
(() => {
	...
	const PERSISTENCE_MAX_RECOVERY_MS = 2 * 60 * 1000
	const PERSISTENCE_MAX_PENDING_BYTES = 256 * 1024 * 1024
	const PERSISTENCE_STOP_RETRY_ATTEMPTS = 8
	const WORKLET_FLUSH_TIMEOUT_MS = 2000
	...
	class PoREBrowserPcmRecorder {
		constructor({ AudioContextClass = window.AudioContext || window.webkitAudioContext, workletUrl = resolveWorkletUrl(), persistenceStoreFactory = () => new window.PoREBrowserPcmPersistenceStore(), persistenceChunkBytes = 128 * 1024, maxPersistenceQueueBytes = PERSISTENCE_MAX_PENDING_BYTES, onPersistenceSafetyStop = null } = {}) {
			...
			this.state = 'idle'; this.context = null; this.source = null; this.worklet = null; this.monitorDestination = null; this.outputGain = null; this.stream = null; this.sampleRate = null; this.channels = 1; this.startedAt = null; this.stoppedAt = null; this.sequence = 0
			this.captureId = null; this.recordingSessionId = null; this.productionId = null; this.productionLabel = null; this.participantLabel = null; this.recordingId = null; this.persistenceStore = null; this.workletFlushPromise = null; this.workletFlushTimer = null; this.pendingParts = []; this.pendingBytes = 0; this.persistedChunkIndex = 0; this.persistenceQueue = []; this.persistenceQueueBytes = 0; this.persistenceDrainPromise = null; this.persistenceRetryTimer = null; this.persistenceRetryDelayMs = PERSISTENCE_RETRY_INITIAL_MS; this.persistenceState = 'idle'; this.persistenceFailureStartedAt = null; this.persistenceSafetyStopEmitted = false; this.openingSignet = null; this.openingSignetReadyPromise = null; this.openingSignetReadyResolve = null; this.closingSignet = null; this.closingSignetReadyPromise = null; this.closingSignetReadyResolve = null; this.capturedSamples = 0; this.receivedAudioBuffers = 0; this.workletProcessorError = null; this.openingTestTonePending = false; this.openingTestToneSamplesWritten = 0
		}
		...
		async start(track, metadata = {}) {
			...
			try {
				...
				this.worklet.onprocessorerror = event => {
					...
					window.dispatchEvent(new CustomEvent('pore:recording-error', { detail: { error: new Error('PoRE AudioWorklet processor failed') } }))
				}
				this.worklet.port.onmessage = event => {
					if (event.data instanceof Float32Array) {
						this.receivedAudioBuffers += 1
						if (this.state === 'recording' || this.state === 'stopping') this._acceptSamples(event.data)
						return
					}
				...
				this.worklet.connect(this.monitorDestination)
				if (this.context.state === 'suspended') await this.context.resume()
				console.debug('[NC-PoRe] PCM recorder active', {
				...
				this.state = 'error'
				try {
					if (this.captureId && this.persistenceStore) await this.persistenceStore.removeCapture(this.captureId)
				} catch (cleanupError) {
					console.error('[NC-PoRe] Failed to remove incomplete browser capture after start failure', cleanupError)
				}
				await this._cleanup()
				throw error
			}
		...
		_acceptSamples(samples) {
			...
			if (this.closingTestTonePending) {
				const sampleOffset = this.capturedSamples
				const toneSamples = Math.round(this.sampleRate * OPENING_TEST_TONE_MS / 1000)
				const remaining = toneSamples - this.closingTestToneSamplesWritten
				const tone = Math.min(remaining, samples.length)
				for (let i = 0; i < tone; i += 1) {
					const sampleIndex = this.closingTestToneSamplesWritten + i
					samples[i] = Math.max(-1, Math.min(1, samples[i] + OPENING_TEST_TONE_AMPLITUDE * Math.sin(2 * Math.PI * 800 * (sampleIndex / this.sampleRate))))
				}
			...
			this.persistenceDrainPromise = (async () => {
				while (this.persistenceQueue.length) {
					...
					try {
						...
						if (!this.persistenceQueue.length) this.persistenceFailureStartedAt = null
						this._publishPersistenceState(this.persistenceQueue.length ? 'degraded' : 'healthy')
					} catch (error) {
						if (this.persistenceFailureStartedAt === null) this.persistenceFailureStartedAt = Date.now()
						this._publishPersistenceState('recovering', error)
						if (Date.now() - this.persistenceFailureStartedAt >= PERSISTENCE_MAX_RECOVERY_MS
							|| this.persistenceQueueBytes >= this.maxPersistenceQueueBytes) {
							this._emitPersistenceSafetyStop(error)
						}
		...
		async stop(reason = 'host') {
			...
			console.debug('[NC-PoRe] PCM recorder stopping', {
				captureId: this.captureId,
				recordingId: this.recordingId,
				receivedAudioBuffers: this.receivedAudioBuffers,
		...
		async _flushWorkletOutput() {
			if (!this.worklet?.port) return
			if (this.workletFlushPromise) return this.workletFlushPromise
			this.workletFlushPromise = new Promise((resolve, reject) => {
				this._pendingWorkletFlushResolve = resolve
				this._pendingWorkletFlushReject = reject
				this.workletFlushTimer = window.setTimeout(() => {
					this.workletFlushTimer = null
					this.workletFlushPromise = null
					this._pendingWorkletFlushResolve = null
					this._pendingWorkletFlushReject = null
					reject(new Error('PoRE AudioWorklet flush timed out'))
				}, WORKLET_FLUSH_TIMEOUT_MS)
				this.worklet.port.postMessage({ type: 'flush' })
			})
			return this.workletFlushPromise
		}

		_resolveWorkletFlush() {
			if (!this.workletFlushPromise) return
			if (this.workletFlushTimer) window.clearTimeout(this.workletFlushTimer)
			this.workletFlushTimer = null
			const resolve = this._pendingWorkletFlushResolve
			this._pendingWorkletFlushResolve = null
			this._pendingWorkletFlushReject = null
			this.workletFlushPromise = null
			resolve?.()
