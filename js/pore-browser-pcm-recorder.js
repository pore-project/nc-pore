...
(() => {
	...
	class PoREBrowserPcmRecorder {
		constructor({ AudioContextClass = window.AudioContext || window.webkitAudioContext, workletUrl = 'pore-browser-pcm-worklet.js', persistenceStoreFactory = () => new window.PoREBrowserPcmPersistenceStore(), persistenceChunkBytes = 128 * 1024 } = {}) {
			...
			this.captureId = null; this.recordingSessionId = null; this.productionId = null; this.recordingId = null; this.persistenceStore = null; this.persistenceChain = Promise.resolve(); this.pendingParts = []; this.pendingBytes = 0; this.persistedChunkIndex = 0
		}
		...
		async start(track, metadata = {}) {
			...
			this.startedAt = new Date().toISOString(); this.stoppedAt = null; this.sequence += 1; this.stream = new MediaStream([track]); this.captureId = metadata.captureId || technicalId('browser-capture'); this.recordingSessionId = metadata.recordingSessionId || technicalId('browser-session'); this.productionId = metadata.productionId || null; this.recordingId = metadata.recordingId || null; this.pendingParts = []; this.pendingBytes = 0; this.persistedChunkIndex = 0; this.persistenceChain = Promise.resolve(); this.persistenceStore = metadata.persistenceStore || this.persistenceStoreFactory()
			try {
				...
				await this.persistenceStore.beginCapture({ captureId: this.captureId, recordingSessionId: this.recordingSessionId, productionId: this.productionId, recordingId: this.recordingId, sequence: this.sequence, startedAt: this.startedAt, sampleRate: this.sampleRate, channels: this.channels, encoding: 'pcm_s24le', format: 'audio/wav' })
