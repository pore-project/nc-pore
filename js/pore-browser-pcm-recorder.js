...
(() => {
	...
	class PoREBrowserPcmRecorder {
		...
		async start(track) {
			...
			try {
				this.context = new this.AudioContextClass(); await this.context.audioWorklet.addModule(this.workletUrl); this.sampleRate = this.context.sampleRate; this.source = this.context.createMediaStreamSource(this.stream); this.worklet = new AudioWorkletNode(this.context, 'pore-pcm-processor', { numberOfInputs: 1, numberOfOutputs: 1, channelCount: 1, channelCountMode: 'explicit', channelInterpretation: 'speakers' })
				this.worklet.port.onmessage = event => { if (this.state === 'recording' && event.data instanceof Float32Array) this.parts.push(float32ToPcm24(event.data)) }
				...
				window.dispatchEvent(new CustomEvent('pore:recording-started', { detail: { sequence: this.sequence, startedAt: this.startedAt, sampleRate: this.sampleRate, channels: this.channels, format: 'pcm_s24le', trackId: track.id, trackLabel: track.label } }))
		...
		async stop(reason = 'host') {
			...
			try { this.worklet?.disconnect(); this.source?.disconnect(); if (this.context?.state !== 'closed') await this.context?.close(); const pcm = concatUint8Arrays(this.parts); const blob = new Blob([createWavHeader(pcm.length, this.sampleRate, this.channels), pcm], { type: 'audio/wav' }); const artifact = { kind: 'audio', format: 'audio/wav', encoding: 'pcm_s24le', size: blob.size, sequence: this.sequence, startedAt: this.startedAt, stoppedAt: this.stoppedAt, stopReason: reason, sampleRate: this.sampleRate, channels: this.channels, blob }; await this._cleanup(); this.state = 'idle'; window.dispatchEvent(new CustomEvent('pore:recording-finalized', { detail: artifact })); return artifact } catch (error) { this.state = 'error'; await this._cleanup(); window.dispatchEvent(new CustomEvent('pore:recording-error', { detail: { error } })); throw error }
		}
		async _cleanup() { if (this.stream) this.stream.getTracks().forEach(track => track.stop()); this.stream = null; this.source = null; this.worklet = null; this.context = null; this.parts = [] }
	}
