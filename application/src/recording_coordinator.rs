...
where
    R: ProductionSessionRepository,
{
    pub fn new(repository: &'a mut R, session_id: ProductionId, actor_id: ParticipantId, recording_id: RecordingId) -> Self {
    ...
    pub fn ensure_session(&mut self, owner_id: ParticipantId, participants: impl IntoIterator<Item = ParticipantId>) -> Result<(), ProductionSessionError> {
        ...
        for participant_id in participants {
            ...
            add_participation_to_production_session(
                ...
                | crate::session::AddParticipationToProductionSessionError::Repository(_) => ProductionSessionError::InvalidStateTransition,
            })?;
        }
        ...
        if session.status() == nc_pore_core::session::ProductionStatus::Created {
            start_production_session(self.repository, &self.session_id, &owner_id).map_err(|error| match error {
                ...
                | crate::session::StartProductionSessionError::Repository(_) => ProductionSessionError::InvalidStateTransition,
            })?;
        }
        Ok(())
    }
    ...
    pub fn ensure_recording(&mut self) -> Result<(), ProductionSessionError> {
        let session = self.repository.get(&self.session_id)
            .map_err(|_| ProductionSessionError::InvalidStateTransition)?
            .ok_or(ProductionSessionError::InvalidStateTransition)?;
        if session.recordings().iter().any(|recording| recording.id() == &self.recording_id) {
            return Ok(());
        }
        add_recording_to_production_session(self.repository, &self.session_id, &self.actor_id, Recording::new(self.recording_id.value()))
            ...
            .map_err(|error| match error {
                ...
                | crate::session::AddRecordingToProductionSessionError::Repository(_) => ProductionSessionError::InvalidStateTransition,
            })
    }
    ...
    pub fn mark_ready(&mut self) -> Result<ClientRecordingState, ProductionSessionError> {
        let actor_id = self.actor_id.clone();
        let recording_id = self.recording_id.clone();
        self.mutate(|session| session.mark_recording_ready_by(&actor_id, &recording_id).map(|_| ()))?;
        self.snapshot()
    }
    ...
    pub fn complete(&mut self, artifact_id: impl Into<String>) -> Result<ClientRecordingState, ProductionSessionError> {
        ...
        self.mutate(|session| session.complete_recording_by(&actor_id, &recording_id, artifact_id))?;
        self.snapshot()
    }
    ...
    fn load_session(&self) -> Result<nc_pore_core::session::ProductionSession, ProductionSessionError> {
        self.repository.get(&self.session_id)
            .map_err(|_| ProductionSessionError::InvalidStateTransition)?
            .ok_or(ProductionSessionError::InvalidStateTransition)
    }
