...
impl<'a, R: ProductionSessionRepository> RecordingCoordinator<'a, R> {
    ...
    pub fn ensure_session(
        ...
        if session.status() == nc_pore_core::session::ProductionStatus::Created {
            start_production_session(self.repository, &self.session_id, &owner_id).map_err(
                |error| match error {
    ...
    pub fn ensure_recording(&mut self) -> Result<(), ProductionSessionError> {
        ...
        if session
            .recordings()
            .iter()
            .any(|recording| recording.id() == &self.recording_id)
        {
            return Ok(());
        }
    ...
    pub fn begin(
        ...
        let recording = self.recording_id.clone();
        self.mutate(|session| session.begin_recording_by(&actor, &recording, participants))?;
        self.snapshot()
    }
    ...
    pub fn mark_ready(&mut self) -> Result<ClientRecordingState, ProductionSessionError> {
        ...
        self.mutate(|session| {
            session
                .mark_recording_ready_by(&actor, &recording)
                .map(|_| ())
        })?;
        self.snapshot()
    }
    ...
    pub fn start(&mut self) -> Result<ClientRecordingState, ProductionSessionError> {
        let actor = self.actor_id.clone();
        let recording = self.recording_id.clone();
        self.mutate(|session| session.start_recording_by(&actor, &recording))?;
        self.snapshot()
    }
    ...
    pub fn request_stop(&mut self) -> Result<ClientRecordingState, ProductionSessionError> {
        let actor = self.actor_id.clone();
        let recording = self.recording_id.clone();
        self.mutate(|session| session.stop_recording_by(&actor, &recording))?;
        self.snapshot()
    }
    ...
    pub fn acknowledge_stop(&mut self) -> Result<ClientRecordingState, ProductionSessionError> {
        let actor = self.actor_id.clone();
        let recording = self.recording_id.clone();
        self.mutate(|session| session.acknowledge_recording_stop_by(&actor, &recording))?;
        self.snapshot()
    }
    ...
    pub fn complete(
        ...
        let actor = self.actor_id.clone();
        let recording = self.recording_id.clone();
        let artifact = RecordingArtifactId::new(artifact_id.into());
        self.mutate(|session| session.complete_recording_by(&actor, &recording, artifact))?;
        self.snapshot()
    }
...
mod tests {
    ...
    fn coordinator_can_establish_session_and_participants() {
        ...
        assert_eq!(
            repository.sessions[0].status(),
            nc_pore_core::session::ProductionStatus::Active
        );
    }
