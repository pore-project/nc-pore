...
impl<'a, R: ProductionSessionRepository> RecordingCoordinator<'a, R> {
    pub fn new(
        ...
        Self { repository, session_id, actor_id, recording_id }
    }
    ...
    pub fn ensure_session(
        ...
        for participant_id in participants {
            let exists = self.repository.get(&self.session_id)
                .map_err(|_| ProductionSessionError::InvalidStateTransition)?
                .map(|session| session.participations().iter().any(|participation| participation.participant_id == participant_id))
                .unwrap_or(false);
            if exists { continue; }
        ...
        let session = self.repository.get(&self.session_id)
            .map_err(|_| ProductionSessionError::InvalidStateTransition)?
    ...
    pub fn ensure_recording(&mut self) -> Result<(), ProductionSessionError> {
        ...
        add_recording_to_production_session(self.repository, &self.session_id, &self.actor_id, Recording::new(self.recording_id.value()))
            .map(|_| ())
            .map_err(|error| match error {
    ...
    pub fn begin(&mut self, participants: impl IntoIterator<Item = ParticipantId>) -> Result<ClientRecordingState, ProductionSessionError> {
        let actor = self.actor_id.clone(); let recording = self.recording_id.clone();
        self.mutate(|session| session.begin_recording_by(&actor, &recording, participants))?; self.snapshot()
    }
    pub fn mark_ready(&mut self) -> Result<ClientRecordingState, ProductionSessionError> {
        let actor = self.actor_id.clone(); let recording = self.recording_id.clone();
        self.mutate(|session| session.mark_recording_ready_by(&actor, &recording).map(|_| ()))?; self.snapshot()
    }
    pub fn start(&mut self) -> Result<ClientRecordingState, ProductionSessionError> {
        let actor = self.actor_id.clone(); let recording = self.recording_id.clone();
        self.mutate(|session| session.start_recording_by(&actor, &recording))?; self.snapshot()
    }
    pub fn request_stop(&mut self) -> Result<ClientRecordingState, ProductionSessionError> {
        let actor = self.actor_id.clone(); let recording = self.recording_id.clone();
        self.mutate(|session| session.stop_recording_by(&actor, &recording))?; self.snapshot()
    }
    pub fn acknowledge_stop(&mut self) -> Result<ClientRecordingState, ProductionSessionError> {
        let actor = self.actor_id.clone(); let recording = self.recording_id.clone();
        self.mutate(|session| session.acknowledge_recording_stop_by(&actor, &recording))?; self.snapshot()
    }
    pub fn complete(&mut self, artifact_id: impl Into<String>) -> Result<ClientRecordingState, ProductionSessionError> {
        let actor = self.actor_id.clone(); let recording = self.recording_id.clone();
        self.mutate(|session| session.complete_recording_by(&actor, &recording, RecordingArtifactId::new(artifact_id.into())))?; self.snapshot()
    }
    ...
    where F: FnOnce(&mut nc_pore_core::session::ProductionSession) -> Result<(), ProductionSessionError> {
        let mut session = self.repository.get(&self.session_id)
            .map_err(|_| ProductionSessionError::InvalidStateTransition)?
            .ok_or(ProductionSessionError::InvalidStateTransition)?;
        mutate(&mut session)?;
...
mod tests {
    ...
    impl ProductionSessionRepository for InMemoryRepository {
        ...
        fn update(&mut self, session: &ProductionSession) -> Result<(), Self::Error> { let existing = self.sessions.iter_mut().find(|existing| existing.id == session.id).ok_or("session not found")?; *existing = session.clone(); Ok(()) }
        fn get(&self, id: &ProductionId) -> Result<Option<ProductionSession>, Self::Error> { Ok(self.sessions.iter().find(|session| &session.id == id).cloned()) }
    }
