...
impl CaptureProvider for TimedCpalCaptureProvider {
    fn start_capture(&mut self, configuration: &RecordingConfiguration) -> Result<(), CaptureStartError> {
...
fn main() -> std::io::Result<()> {
    ...
    client.create(SESSION_ID, OWNER_ID).expect("vertical-slice session must be creatable");
    drop(client);

    let production_id = ProductionId::new(SESSION_ID);
    let owner = ParticipantId::new(OWNER_ID);
    let mut session = repository.get(&production_id).expect("session lookup must succeed").expect("vertical-slice session must exist");
    session.start_by(&owner).expect("vertical-slice session must be active before recording");
    session.add_recording_by(&owner, Recording::new(RECORDING_ID)).expect("vertical-slice recording must be creatable");
    repository.update(&session).expect("vertical-slice session update must succeed");
    ...
    let mut state = ServerState { repository, ready_participants: HashSet::new(), recorder };
...
fn handle_connection(mut stream: TcpStream, state: &mut ServerState) {
    ...
    let bytes_read = match stream.read(&mut buffer) {
        ...
        Err(error) => { eprintln!("request read error: {error}"); return; }
    };
    ...
    let request_line = match headers.lines().next() { Some(line) => line, None => return };
    ...
    let response = format!("HTTP/1.1 {status} {reason}\r\nContent-Type: {content_type}\r\nContent-Length: {}\r\nConnection: close\r\n\r\n{body}", body.len(), reason = reason_phrase(status));
    if let Err(error) = stream.write_all(response.as_bytes()) { eprintln!("response write error: {error}"); }
}
...
fn join_session(state: &mut ServerState, body: &str) -> (u16, &'static str, String) {
    let participant_id = match json_field(body, "participant_id") { Some(value) if !value.is_empty() => value, _ => return response(400, r#"{"error":"invalid_request"}"#) };
...
fn mark_ready(state: &mut ServerState, body: &str) -> (u16, &'static str, String) {
    ...
    if context.state == SessionState::Completed || !context.capabilities.contains(&SessionCapability::ParticipateInRecording) {
        return response(409, r#"{"error":"not_ready_for_recording"}"#);
    }
    state.ready_participants.insert(participant_id.clone());
    (200, "application/json; charset=utf-8", format!("{{\"session_id\":\"{}\",\"actor_id\":\"{}\",\"state\":\"READY\"}}", json_escape(SESSION_ID), json_escape(&participant_id)))
}

fn record(state: &mut ServerState, body: &str) -> (u16, &'static str, String) {
    let participant_id = match json_field(body, "participant_id") { Some(value) if !value.is_empty() => value, _ => return response(400, r#"{"error":"invalid_request"}"#) };
    if !state.ready_participants.contains(&participant_id) { return response(409, r#"{"error":"participant_not_ready"}"#); }
    let mut recorder = std::mem::replace(&mut state.recorder, new_recorder());
    let result = execute_recording(&mut state.repository, &ProductionId::new(SESSION_ID), &ParticipantId::new(&participant_id), &RecordingId::new(RECORDING_ID), &mut recorder, &RecordingConfiguration::default());
    match result {
        Ok(artifact) => {
            let payload_bytes: usize = artifact.tracks().iter().flat_map(|track| track.chunks()).map(|chunk| chunk.payload().data().len()).sum();
            (200, "application/json; charset=utf-8", format!("{{\"recording_id\":\"{}\",\"artifact_id\":\"{}\",\"tracks\":{},\"payload_bytes\":{},\"state\":\"COMPLETED\"}}", RECORDING_ID, json_escape(artifact.id.value()), artifact.tracks().len(), payload_bytes))
        }
...
fn new_recorder() -> Recorder {
    ...
    let capture = TimedCpalCaptureProvider { provider: CpalCaptureProvider::new(), duration: CAPTURE_DURATION };
    RecorderApplication::new(RecordingSession::new(RECORDING_ID), capture, processor)
}

fn response(status: u16, body: impl Into<String>) -> (u16, &'static str, String) { (status, "application/json; charset=utf-8", body.into()) }
...
fn reason_phrase(status: u16) -> &'static str { match status { 200 => "OK", 400 => "Bad Request", 404 => "Not Found", 409 => "Conflict", 500 => "Internal Server Error", _ => "Unknown" } }
fn session_json(session: &nc_pore_application::client::ClientProductionSession) -> String {
    let participants = session.participants.iter().map(|participant| { let roles = participant.roles.iter().map(|role| format!("\"{}\"", role_name(*role))).collect::<Vec<_>>().join(","); format!("{{\"id\":\"{}\",\"roles\":[{}]}}", json_escape(&participant.id), roles) }).collect::<Vec<_>>().join(",");
...
fn role_name(role: ClientRole) -> &'static str { match role { ClientRole::Owner => "Owner", ClientRole::Producer => "Producer", ClientRole::Participant => "Participant", ClientRole::Guest => "Guest" } }
fn capability_name(capability: SessionCapability) -> &'static str { match capability { SessionCapability::StartSession => "StartSession", SessionCapability::CompleteSession => "CompleteSession", SessionCapability::ManageParticipants => "ManageParticipants", SessionCapability::ManageRecordings => "ManageRecordings", SessionCapability::ParticipateInRecording => "ParticipateInRecording" } }
fn json_escape(value: &str) -> String { value.replace('\\', "\\\\").replace('"', "\\\"").replace('\n', "\\n").replace('\r', "\\r").replace('\t', "\\t") }

const INDEX_HTML: &str = r#"<!doctype html>
<html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>NC-PoRe Session Recording</title></head><body>
<h1>NC-PoRe session recording vertical slice</h1><p id="status">Opening session…</p><button id="join">Join as Bob</button><button id="ready" disabled>Ready</button><button id="record" disabled>Start Recording</button><pre id="result"></pre>
<script>
...
async function refresh() { try { const context = await api(`/api/sessions/${session}/context?actor=${encodeURIComponent(actor)}`); result.textContent = JSON.stringify(context, null, 2); status.textContent = `Session ${context.state}; actor ${context.actor_id}`; ready.disabled = !context.capabilities.includes('ParticipateInRecording'); } catch (error) { status.textContent = `Session not joined: ${error}`; } }
join.onclick = async () => { try { await api(`/api/sessions/${session}/join`, { method: 'POST', headers: {'Content-Type': 'application/json'}, body: JSON.stringify({participant_id: actor}) }); join.disabled = true; await refresh(); } catch (error) { status.textContent = `Join failed: ${error}`; } };
ready.onclick = async () => { try { const value = await api(`/api/sessions/${session}/ready`, { method: 'POST', headers: {'Content-Type': 'application/json'}, body: JSON.stringify({participant_id: actor}) }); result.textContent = JSON.stringify(value, null, 2); status.textContent = 'Bob = READY'; ready.disabled = true; record.disabled = false; } catch (error) { status.textContent = `Ready failed: ${error}`; } };
record.onclick = async () => { try { record.disabled = true; status.textContent = 'Recording…'; const value = await api(`/api/sessions/${session}/record`, { method: 'POST', headers: {'Content-Type': 'application/json'}, body: JSON.stringify({participant_id: actor}) }); result.textContent = JSON.stringify(value, null, 2); status.textContent = 'Recording COMPLETED'; } catch (error) { status.textContent = `Recording failed: ${error}`; record.disabled = false; } };
refresh();
</script></body></html>"#;
...
mod tests {
    ...
    fn state() -> ServerState {
        ...
        ServerState { repository, ready_participants: HashSet::new(), recorder: new_recorder() }
    }
    #[test]
    fn participant_must_be_ready_before_recording() { let mut state = state(); let result = record(&mut state, r#"{"participant_id":"bob-1"}"#); assert_eq!(result.0, 409); assert!(result.2.contains("participant_not_ready")); }
    #[test]
    fn participant_can_cross_ready_boundary() { let mut state = state(); assert_eq!(join_session(&mut state, r#"{"participant_id":"bob-1"}"#).0, 200); let ready = mark_ready(&mut state, r#"{"participant_id":"bob-1"}"#); assert_eq!(ready.0, 200); assert!(state.ready_participants.contains("bob-1")); }
}
