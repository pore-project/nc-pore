use nc_pore_application::browser_recording_handoff::{BrowserRecordingHandoff, BrowserRecordingStopReason};
...
fn main() -> std::io::Result<()> {
    ...
    let mut state = State { repo, recording: None, persistence: InMemoryPersistenceProvider::new(), persisted_artifact_id: None };
...
fn handle(mut stream: TcpStream, state: &mut State) {
    ...
    loop {
        match stream.read(&mut buf) {
            Ok(0) => break,
            Ok(n) => { request.extend_from_slice(&buf[..n]); if request.len() > MAX_BODY + 16 * 1024 { write_response(&mut stream, json(413, "request_too_large")); return } if complete_request(&request) { break } }
            Err(_) => return,
        }
    }
    ...
    let result = match (method, path) {
        ...
        _ => (404, "application/json; charset=utf-8", String::from("{\"error\":\"not_found\"}")),
    };
...
fn complete_request(buf: &[u8]) -> bool {
    ...
    let n = h.lines().find_map(|l| { let (a, b) = l.split_once(':')?; (a.eq_ignore_ascii_case("content-length")).then(|| b.trim().parse::<usize>().ok()) }).flatten().unwrap_or(0);
    buf.len() >= i + 4 + n
}
...
fn join(st: &mut State, b: &str) -> (u16, &'static str, String) {
    ...
    match c.add_participant(SESSION_ID, OWNER_ID, &id, [ClientRole::Participant]) { Ok(_) => json(200, "ok"), Err(ClientSessionError::ParticipantAlreadyExists) => json(409, "participant_already_exists"), Err(_) => json(409, "join_rejected") }
}
fn start(st: &mut State) -> (u16, &'static str, String) { let mut c = ClientSessionService::new(&mut st.repo); if c.start(SESSION_ID, OWNER_ID).is_ok() { json(200, "ok") } else { json(409, "start_rejected") } }
fn begin(st: &mut State, b: &str) -> (u16, &'static str, String) {
    let Some(a) = field(b, "actor_id") else { return json(400, "invalid_request") };
    let a = ParticipantId::new(a);
    if a.value() != OWNER_ID { return json(403, "owner_required") }
    if st.recording.is_some() { return json(409, "recording_already_active") }
    let pid = ProductionId::new(SESSION_ID);
    let rid = nc_pore_core::recording::RecordingId::new(RECORDING_ID);
    let Some(mut s) = st.repo.get(&pid).ok().flatten() else { return json(404, "session_not_found") };
    if !s.recordings().iter().any(|r| r.id() == &rid) {
        if s.add_recording_by(&a, Recording::new(RECORDING_ID)).is_err() { return json(409, "recording_rejected") }
        if st.repo.update(&s).is_err() { return json(500, "repository_error") }
    }
    match begin_distributed_recording(&mut st.repo, &pid, &a, &rid) { Ok(r) => { let out = recording_json(&r); st.recording = Some(r); (200, "application/json; charset=utf-8", out) }, Err(_) => json(409, "recording_begin_rejected") }
}
fn ready(st: &mut State, b: &str) -> (u16, &'static str, String) {
    let Some(id) = field(b, "participant_id") else { return json(400, "invalid_request") };
    let p = ParticipantId::new(id);
    let Some(r) = st.recording.as_mut() else { return json(409, "recording_not_started") };
    if mark_distributed_recording_ready(&mut st.repo, r, &p).is_ok() { (200, "application/json; charset=utf-8", recording_json(r)) } else { json(409, "ready_rejected") }
}
fn open_recording(st: &mut State, b: &str) -> (u16, &'static str, String) {
    if field(b, "actor_id").as_deref() != Some(OWNER_ID) { return json(403, "owner_required") }
    let Some(r) = st.recording.as_mut() else { return json(409, "recording_not_started") };
    if r.trigger_opening().is_ok() { (200, "application/json; charset=utf-8", recording_json(r)) } else { json(409, "opening_rejected") }
}
fn opening_confirm(st: &mut State, b: &str) -> (u16, &'static str, String) {
    let Some(id) = field(b, "participant_id") else { return json(400, "invalid_request") };
    let p = ParticipantId::new(id);
    let Some(r) = st.recording.as_mut() else { return json(409, "recording_not_started") };
    if confirm_distributed_recording_opening(&mut st.repo, r, &p).is_ok() { (200, "application/json; charset=utf-8", recording_json(r)) } else { json(409, "opening_confirmation_rejected") }
}
fn stop(st: &mut State, b: &str) -> (u16, &'static str, String) {
    ...
    if s.stop_recording_by(&a, r.recording_id()).is_err() { return json(409, "stop_rejected") }
    if st.repo.update(&s).is_err() { return json(500, "repository_error") }
...
fn stop_ack(st: &mut State, b: &str) -> (u16, &'static str, String) {
    let Some(id) = field(b, "participant_id") else { return json(400, "invalid_request") };
    let p = ParticipantId::new(id);
    let Some(r) = st.recording.as_mut() else { return json(409, "recording_not_started") };
...
fn finalize_recording(st: &mut State, b: &str) -> (u16, &'static str, String) {
    ...
    let (Some(rid), Some(tid), Some(fmt), Some(hex)) = (field(b, "recording_id"), field(b, "track_id"), field(b, "format"), field(b, "payload_hex")) else { return json(400, "invalid_request") };
    let Ok(payload) = decode_hex(&hex) else { return json(400, "invalid_payload") };
    let handoff = BrowserRecordingHandoff::new(rid, tid, payload, fmt, BrowserRecordingStopReason::UserRequested);
    match handoff.persist(RecordingSessionId::new(SESSION_ID), &mut st.persistence) {
        Ok(artifact) => {
            st.persisted_artifact_id = Some(artifact.id.value().to_owned());
            (200, "application/json; charset=utf-8", format!("{{\"artifact_id\":\"{}\",\"status\":\"{:?}\"}}", escape(artifact.id.value()), artifact.status()))
        }
...
fn state_json(st: &mut State) -> (u16, &'static str, String) {
    if let Some(r) = st.recording.as_ref() { (200, "application/json; charset=utf-8", recording_json(r)) }
        ...
        let p = s.participations().iter().map(|p| format!("\"{}\"", escape(p.participant_id.value()))).collect::<Vec<_>>().join(",");
        (200, "application/json; charset=utf-8", format!("{{\"session_status\":\"{:?}\",\"participants\":[{}],\"recording_status\":null,\"workflow\":null}}", s.status(), p))
    } else { json(200, "no_session") }
}
fn recording_json(r: &DistributedRecording) -> String {
    let c = r.workflow().coordination();
    let list = |v: &[ParticipantId]| v.iter().map(|p| format!("\"{}\"", escape(p.value()))).collect::<Vec<_>>().join(",");
    format!("{{\"recording_status\":\"{:?}\",\"workflow\":\"{:?}\",\"participants\":[{}],\"ready\":[{}],\"opening_confirmed\":[{}],\"stop_acknowledged\":[{}]}}", r.workflow().recording().status(), r.workflow().status(), list(c.participants()), list(c.ready_participants()), list(c.opening_confirmed_participants()), list(c.stop_acknowledged_participants()))
}
fn field(b: &str, n: &str) -> Option<String> { let m = format!("\"{}\":\"", n); let i = b.find(&m)? + m.len(); let r = &b[i..]; Some(r[..r.find('"')?].to_owned()) }
fn decode_hex(v: &str) -> Result<Vec<u8>, ()> { if v.len() % 2 != 0 { return Err(()) } v.as_bytes().chunks_exact(2).map(|p| Ok((hex_value(p[0])? << 4) | hex_value(p[1])?)).collect() }
