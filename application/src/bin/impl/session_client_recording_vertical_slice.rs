...
fn state_json(state:&mut State)->(u16,&'static str,String){if let Some(r)=state.recording.as_ref(){(200,"application/json; charset=utf-8",recording_json(r))}else if let Some(s)=state.repo.get(&ProductionId::new(SESSION_ID)).ok().flatten(){let participants=s.participations().iter().map(|p|format!("\"{}\"",escape(p.participant_id.value()))).collect::<Vec<_>>().join(",");(200,"application/json; charset=utf-8",format!("{{\"session_status\":\"{:?}\",\"participants\":[{participants}],\"recording_status\":null,\"workflow\":null}}",s.status()))}else{json(200,"no_session")}}
