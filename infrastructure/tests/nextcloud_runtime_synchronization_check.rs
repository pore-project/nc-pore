...
#[test]
fn nextcloud_runtime_synchronization_check() {
    let required = [
    ...
    if required.iter().any(|name| env::var(name).is_err()) {
        eprintln!(
            "Nextcloud runtime synchronization check skipped: required credentials are not configured."
        );
        return;
    }
    ...
    let metadata = ArtifactTransferMetadata::new(
        Some("NC-PoRE Runtime Synchronization Check".to_owned()),
    ...
    queue
        ...
        .expect("test artifact must be queued");
    ...
    let result = orchestrator
        .process_next()
        .expect("synchronization orchestration must succeed");
    ...
    assert!(manifest_text.contains("track-runtime-sync"));
    assert!(manifest_text.contains("NC-PoRE Runtime Synchronization Check"));
    ...
    println!(
        "Nextcloud runtime synchronization check passed: orchestration, real transfer, verification and cleanup succeeded for '{root}/{artifact_id}'."
    );
}
