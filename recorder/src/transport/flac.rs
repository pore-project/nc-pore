...
//!
//! PCM is the capture/preservation representation. FLAC is the V1 transport
//! representation: it preserves supported integer PCM samples exactly while
...
use crate::audio::{CaptureChunk, RecordingConfiguration, SampleFormat};
...
/// Encodes interleaved captured integer PCM chunks as one FLAC stream.
///
/// No resampling, channel conversion or bit-depth conversion is performed.
/// F32 is rejected because FLAC transport cannot preserve floating-point
/// samples losslessly.
...
mod tests {
    ...
    fn rejects_partial_samples() {
        ...
        assert_eq!(encode_chunks(&chunks, configuration), Err(FlacEncodeError::InvalidPcmPayload));
    }
    ...
    fn rejects_float_transport() {
        ...
        assert_eq!(encode_chunks(&chunks, configuration), Err(FlacEncodeError::UnsupportedSampleFormat));
    }
}
