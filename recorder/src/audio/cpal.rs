...
impl CpalInputConfiguration {
    ...
    pub fn stream_config_for_sample_rate(&self, sample_rate_hz: u32) -> Option<cpal::StreamConfig> {
        let range = cpal::SupportedStreamConfigRange::new(
            self.channels, self.min_sample_rate_hz, self.max_sample_rate_hz,
            self.buffer_size, self.sample_format,
        );
...
impl CaptureChunkBuffer {
    fn new(configuration: &RecordingConfiguration) -> Self {
        ...
        let frames = configuration.chunk_duration().seconds() as usize
            * configuration.sample_rate_hz() as usize;
        let chunk_size_bytes = frames * bytes_per_sample * usize::from(configuration.channels());
        Self {
            chunks: Vec::new(), current_payload: Vec::new(), next_sequence: 1,
            chunk_size_bytes, sample_format: configuration.sample_format(),
            channels: configuration.channels(), sample_rate_hz: configuration.sample_rate_hz(),
            captured_bytes: 0, pending_signets: Vec::new(),
        }
    }
    ...
    fn finish(mut self) -> Vec<CaptureChunk> {
        ...
        for chunk in self.chunks {
            ...
            for signet in &self.pending_signets {
                mix_signet_into_chunk(&mut payload, start, end, signet.offset_bytes,
                    &signet.payload, self.sample_format);
            }
            chunk_offset = end;
    ...
    for frame in 0..frames {
        ...
        let sample = if active {
            ...
            ((state as f32 / u32::MAX as f32) * 2.0 - 1.0)
                * 0.12 * match signet.kind() { SyncSignetKind::Opening => 1.0, SyncSignetKind::Closing => -1.0 }
        } else { 0.0 };
        for _ in 0..channels {
            match format {
                ...
                SampleFormat::Pcm16 => payload.extend_from_slice(&(sample.clamp(-1.0, 1.0) * 32767.0).round().to_ne_bytes::<2>()),
            }
        }
    }
    payload
...
fn mix_signet_into_chunk(payload: &mut [u8], chunk_start: usize, chunk_end: usize,
    signet_offset: usize, signet_payload: &[u8], format: SampleFormat) {
    let bps = match format { SampleFormat::Pcm16 => 2, SampleFormat::Pcm24 => 3, SampleFormat::F32 => 4 };
    let overlap_start = chunk_start.max(signet_offset);
    let overlap_end = chunk_end.min(signet_offset + signet_payload.len());
    ...
    for absolute in (aligned_start..aligned_end).step_by(bps) {
        ...
        match format {
            SampleFormat::F32 => {
                let a = f32::from_ne_bytes(payload[po..po+4].try_into().unwrap());
                let b = f32::from_ne_bytes(signet_payload[so..so+4].try_into().unwrap());
                payload[po..po+4].copy_from_slice(&(a+b).clamp(-1.0,1.0).to_ne_bytes());
            }
            SampleFormat::Pcm24 => {
                let a = decode_i24(&payload[po..po+3]); let b = decode_i24(&signet_payload[so..so+3]);
                payload[po..po+3].copy_from_slice(&encode_i24_sample(a.saturating_add(b)));
            }
            SampleFormat::Pcm16 => {
...
fn encode_i24_sample(value: i32) -> [u8;3] { let b=value.clamp(-8_388_608,8_388_607).to_ne_bytes(); [b[0],b[1],b[2]] }
fn decode_i24(b: &[u8]) -> i32 { i32::from_ne_bytes([b[0],b[1],b[2],if b[2]&0x80!=0 {0xff} else {0}]) }
...
impl CpalCaptureProvider {
    ...
    pub fn discover_input_configurations(&self) -> Result<Vec<CpalInputConfiguration>, String> {
        ...
        device.supported_input_configs().map_err(|e| format!("Unterstützte Eingabekonfigurationen konnten nicht gelesen werden: {e}"))
            .map(|configs| configs.map(|c| CpalInputConfiguration::from_supported_config(&c)).collect())
    }
...
impl CaptureProvider for CpalCaptureProvider {
    fn start_capture(&mut self, requested: &RecordingConfiguration) -> Result<(), CaptureStartError> {
        ...
        *self.chunk_buffer.lock().unwrap() = Some(CaptureChunkBuffer::new(&actual));
...
pub fn inspect_default_input_device() -> Result<(), String> { let d=cpal::default_host().default_input_device().ok_or_else(|| "Kein Standard-Eingabegerät gefunden.".to_string())?; println!("Standard-Eingabegerät: {d}"); let c=d.default_input_config().map_err(|e|e.to_string())?; println!("Standard-Konfiguration: {} Kanal, {} Hz, {:?}",c.channels(),c.sample_rate(),c.sample_format()); Ok(()) }
