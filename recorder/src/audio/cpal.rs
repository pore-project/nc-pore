...
impl CpalInputConfiguration {
    ...
    pub fn matches_recording_configuration(&self, configuration: &RecordingConfiguration) -> bool {
        let Some(capability) = self.native_capability() else {
            return false;
        };
...
impl CaptureChunkBuffer {
    ...
    fn finish_current_chunk(&mut self) {
        ...
        self.chunks
            .push(CaptureChunk::with_payload(self.next_sequence, payload));
...
fn render_signet(signet: SyncSignet, rate: u32, channels: u16, format: SampleFormat) -> Vec<u8> {
    ...
    for frame in 0..frames {
        ...
        let active = signet.events().iter().any(|event| {
            ms >= u64::from(event.start_ms())
                && ms < u64::from(event.start_ms() + event.duration_ms())
        });
...
fn mix_signet_into_chunk(
    ...
    for absolute in (aligned_start..aligned_end).step_by(bps) {
        let po = absolute - chunk_start;
        let so = absolute - signet_offset;
        match format {
            SampleFormat::F32 => {
            ...
            SampleFormat::Pcm24 => {
                let a = decode_i24(&payload[po..po + 3]);
                let b = decode_i24(&signet_payload[so..so + 3]);
...
fn encode_i24_sample(value: i32) -> [u8; 3] {
    let bytes = value.clamp(-8_388_608, 8_388_607).to_ne_bytes();
    [bytes[0], bytes[1], bytes[2]]
}
...
impl CaptureProvider for CpalCaptureProvider {
    fn start_capture(
        ...
        let selection = select_best_native_capture(requested, &capabilities)
            .ok_or(CaptureStartError::UnsupportedRecordingConfiguration)?;
        let concrete = find_configuration_for_selection(&selection, &configs)
            .ok_or(CaptureStartError::UnsupportedRecordingConfiguration)?;
        let format = match selection.sample_format() {
        ...
        let actual = RecordingConfiguration::with_chunk_duration(
            selection.sample_rate_hz(),
            selection.channels(),
            format,
            requested.chunk_duration(),
        );
        let stream_config = concrete
            .stream_config_for_sample_rate(selection.sample_rate_hz())
            .ok_or(CaptureStartError::UnsupportedRecordingConfiguration)?;
        *self.chunk_buffer.lock().unwrap() = Some(CaptureChunkBuffer::new(&actual));
        *self.capture_error.lock().unwrap() = None;
        let buffer = Arc::clone(&self.chunk_buffer);
        let error = Arc::clone(&self.capture_error);
        let stream = device
            .build_input_stream_raw(
                stream_config,
                concrete.sample_format(),
                move |data, _| {
                    if let Some(b) = buffer.lock().unwrap().as_mut() {
                        b.push_bytes(data.bytes());
                    }
                },
                move |e| {
        ...
        stream
            .play()
            .map_err(|_| CaptureStartError::ConfigurationUnavailable)?;
        self.active_configuration = Some(actual);
    ...
    fn stop_capture(&mut self) -> CaptureResult {
        ...
        let error = self.capture_error.lock().unwrap().take();
        let mut result = match error {
            Some(e) => CaptureResult::failed("cpal-capture", e),
