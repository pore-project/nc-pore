...
mod tests {
    ...
    fn requested(rate: u32, channels: u16, format: SampleFormat) -> RecordingConfiguration {
        RecordingConfiguration::with_chunk_duration(rate, channels, format, RecordingChunkDuration::OneMinute)
    }
    ...
    fn exact_native_profile_wins() {
        ...
        let selected = select_best_native_capture(&requested(48_000, 1, SampleFormat::Pcm24), &capabilities).unwrap();
    ...
    fn native_lower_rate_is_used_instead_of_resampling() {
        let capabilities = [NativeAudioCapability::new(1, 44_100, 44_100, NativeSampleFormat::Pcm24)];
        let selected = select_best_native_capture(&requested(48_000, 1, SampleFormat::Pcm24), &capabilities).unwrap();
    ...
    fn native_stereo_is_retained_when_mono_is_unavailable() {
        let capabilities = [NativeAudioCapability::new(2, 48_000, 48_000, NativeSampleFormat::Pcm24)];
        let selected = select_best_native_capture(&requested(48_000, 1, SampleFormat::Pcm24), &capabilities).unwrap();
