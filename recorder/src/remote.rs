...
impl RemoteArtifactMetadata {
    ...
    pub fn recording_started_at(&self) -> SystemTime { self.recording_started_at }
    pub fn display_name(&self) -> Option<&str> { self.display_name.as_deref() }
}

#[derive(Debug, Clone)]
pub struct RemoteArtifact<'a> { artifact: &'a RecordingArtifact, metadata: RemoteArtifactMetadata }

impl<'a> RemoteArtifact<'a> {
    pub fn new(artifact: &'a RecordingArtifact, recording_started_at: SystemTime, display_name: Option<String>) -> Self {
        Self { artifact, metadata: RemoteArtifactMetadata::new(recording_started_at, display_name) }
    }
    pub fn artifact(&self) -> &'a RecordingArtifact { self.artifact }
    pub fn metadata(&self) -> &RemoteArtifactMetadata { &self.metadata }
}

/// Remote confirmation of the exact prepared artifact payloads.
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct RemoteUploadReceipt { artifact_id: String, manifest_hash: ManifestHash, tracks: Vec<RemoteUploadTrackReceipt> }

#[derive(Debug, Clone, PartialEq, Eq)]
pub struct RemoteUploadTrackReceipt { track_id: String, size_bytes: u64, hash: PayloadHash }

impl RemoteUploadReceipt {
    pub fn new(artifact_id: impl Into<String>, manifest_hash: ManifestHash, tracks: Vec<RemoteUploadTrackReceipt>) -> Self {
        Self { artifact_id: artifact_id.into(), manifest_hash, tracks }
    }
    pub fn artifact_id(&self) -> &str { &self.artifact_id }
    pub fn manifest_hash(&self) -> ManifestHash { self.manifest_hash }
    pub fn tracks(&self) -> &[RemoteUploadTrackReceipt] { &self.tracks }
}

impl RemoteUploadTrackReceipt {
    pub fn new(track_id: impl Into<String>, size_bytes: u64, hash: PayloadHash) -> Self { Self { track_id: track_id.into(), size_bytes, hash } }
    pub fn track_id(&self) -> &str { &self.track_id }
    pub fn size_bytes(&self) -> u64 { self.size_bytes }
    pub fn hash(&self) -> PayloadHash { self.hash }
}
...
pub struct CompletionCoordinator<'a, U> { jobs: &'a FilesystemCompletionJobStore, uploader: U }

impl<'a, U> CompletionCoordinator<'a, U>
where U: RemoteArtifactUploader,
{
    pub fn new(jobs: &'a FilesystemCompletionJobStore, uploader: U) -> Self { Self { jobs, uploader } }

    /// Completes one artifact and marks it uploaded only after exact receipt validation.
    pub fn complete(&mut self, job: &mut CompletionJob, artifact: &RecordingArtifact) -> Result<(), CompletionCoordinatorError<U::Error>> {
        job.begin_upload_preparation().map_err(CompletionCoordinatorError::Job)?;
        self.jobs.save(job).map_err(CompletionCoordinatorError::Job)?;
        let prepared = match job.prepare_upload(artifact) {
            Ok(upload) => upload,
            Err(error) => { let _ = job.mark_retryable_failure(); let _ = self.jobs.save(job); return Err(CompletionCoordinatorError::Job(error)); }
        };
        job.mark_ready_for_upload().map_err(CompletionCoordinatorError::Job)?;
        self.jobs.save(job).map_err(CompletionCoordinatorError::Job)?;
        job.mark_uploading().map_err(CompletionCoordinatorError::Job)?;
        self.jobs.save(job).map_err(CompletionCoordinatorError::Job)?;
        let receipt = match self.uploader.upload(&prepared) {
            Ok(receipt) => receipt,
            Err(error) => { job.mark_retryable_failure().map_err(CompletionCoordinatorError::Job)?; self.jobs.save(job).map_err(CompletionCoordinatorError::Job)?; return Err(CompletionCoordinatorError::Upload(error)); }
        };
        if let Err(error) = validate_upload_receipt(&prepared, &receipt) {
            job.mark_retryable_failure().map_err(CompletionCoordinatorError::Job)?;
            self.jobs.save(job).map_err(CompletionCoordinatorError::Job)?;
            return Err(CompletionCoordinatorError::Confirmation(error));
        }
        job.mark_uploaded().map_err(CompletionCoordinatorError::Job)?;
        self.jobs.save(job).map_err(CompletionCoordinatorError::Job)?;
        Ok(())
    }
    ...
    pub fn complete_from_preservation(&mut self, job: &mut CompletionJob, preservation: &FilesystemPreservationStore, recording_session_id: RecordingSessionId) -> Result<(), CompletionCoordinatorError<U::Error>> {
        let preserved = match preservation.load(job.capture_id()) {
            PreservationLoadResult::Valid(capture) => capture,
            PreservationLoadResult::Incomplete => return Err(CompletionCoordinatorError::Preservation(PreservationCompletionError::Incomplete)),
            PreservationLoadResult::Inconsistent => return Err(CompletionCoordinatorError::Preservation(PreservationCompletionError::Inconsistent)),
            PreservationLoadResult::NotFound => return Err(CompletionCoordinatorError::Preservation(PreservationCompletionError::NotFound)),
        };
        let artifact = RecordingArtifactFactory::create(preserved, recording_session_id);
        self.complete(job, &artifact)
    }

    pub fn uploader(&self) -> &U { &self.uploader }
}

#[derive(Debug, PartialEq, Eq)]
pub enum CompletionCoordinatorError<E> { Job(CompletionJobError), Preservation(PreservationCompletionError), Upload(E), Confirmation(RemoteUploadValidationError) }

#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum PreservationCompletionError { Incomplete, Inconsistent, NotFound }

/// Validates that the remote receipt confirms exactly the prepared upload.
pub fn validate_upload_receipt(upload: &PreparedUpload, receipt: &RemoteUploadReceipt) -> Result<(), RemoteUploadValidationError> {
    if upload.artifact_id() != receipt.artifact_id() { return Err(RemoteUploadValidationError::ArtifactMismatch); }
    if upload.manifest_hash() != receipt.manifest_hash() { return Err(RemoteUploadValidationError::ManifestMismatch); }
    if upload.tracks().len() != receipt.tracks().len() { return Err(RemoteUploadValidationError::TrackMismatch); }
    for (prepared, confirmed) in upload.tracks().iter().zip(receipt.tracks()) {
        if prepared.track_id() != confirmed.track_id() || prepared.size_bytes() != confirmed.size_bytes() || prepared.hash() != confirmed.hash() { return Err(RemoteUploadValidationError::TrackMismatch); }
    }
    Ok(())
}

#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum RemoteUploadValidationError { ArtifactMismatch, ManifestMismatch, TrackMismatch }

fn normalize_display_name(display_name: Option<String>) -> Option<String> {
    display_name.and_then(|name| { let trimmed = name.trim(); (!trimmed.is_empty()).then(|| trimmed.to_owned()) })
}
...
mod tests {
    ...
    fn test_upload() -> PreparedUpload {
        ...
        track.add_chunk(RecordingChunk::with_payload(1, "track-a/chunk-000001", vec![0, 0, 1, 0, 2, 0, 3, 0]));
    ...
    fn test_artifact() -> RecordingArtifact {
        ...
        artifact.add_track(track); artifact
    }
    ...
    impl RemoteArtifactUploader for ConfirmingUploader {
        type Error = ();
        fn upload(&mut self, upload: &PreparedUpload) -> Result<RemoteUploadReceipt, Self::Error> {
            let tracks = upload.tracks().iter().map(|track| RemoteUploadTrackReceipt::new(track.track_id(), track.size_bytes(), track.hash())).collect();
            Ok(RemoteUploadReceipt::new(upload.artifact_id(), upload.manifest_hash(), tracks))
        }
    }
    ...
    impl RemoteArtifactUploader for MismatchingUploader {
        type Error = ();
        fn upload(&mut self, upload: &PreparedUpload) -> Result<RemoteUploadReceipt, Self::Error> {
            Ok(RemoteUploadReceipt::new(upload.artifact_id(), upload.manifest_hash(), vec![RemoteUploadTrackReceipt::new("track-a", 0, PayloadHash::from_bytes(b"wrong"))]))
        }
    }
    ...
    fn receipt_rejects_wrong_payload_hash() {
        let upload = test_upload();
        let receipt = RemoteUploadReceipt::new(upload.artifact_id(), upload.manifest_hash(), vec![RemoteUploadTrackReceipt::new(upload.tracks()[0].track_id(), upload.tracks()[0].size_bytes(), PayloadHash::from_bytes(b"wrong"))]);
        assert_eq!(validate_upload_receipt(&upload, &receipt), Err(RemoteUploadValidationError::TrackMismatch));
    }
    ...
    fn coordinator_rejects_mismatching_confirmation() {
        ...
        assert_eq!(error, CompletionCoordinatorError::Confirmation(RemoteUploadValidationError::TrackMismatch));
    ...
    fn coordinator_can_complete_from_durable_preservation() {
        ...
        coordinator.complete_from_preservation(&mut job, &preservation, RecordingSessionId::new("session-056")).unwrap();
        assert_eq!(job.state(), crate::completion::CompletionJobState::Uploaded);
    ...
    fn missing_durable_capture_is_not_treated_as_upload_failure() {
        ...
        let mut coordinator = CompletionCoordinator::new(&jobs, ConfirmingUploader);
        let error = coordinator.complete_from_preservation(&mut job, &preservation, RecordingSessionId::new("session-057")).unwrap_err();
        assert_eq!(error, CompletionCoordinatorError::Preservation(PreservationCompletionError::NotFound));
    ...
    fn display_name_is_preserved() {
        let metadata = RemoteArtifactMetadata::new(SystemTime::UNIX_EPOCH, Some(" Interview Frizz Feick ".into()));
