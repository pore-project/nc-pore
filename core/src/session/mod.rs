...
pub enum ProductionStatus { Created, Active, Completed }
...
impl ProductionSession {
    pub fn new(id: ProductionId) -> Self { Self::new_with_actor(id, None) }

    pub fn new_with_actor(id: ProductionId, actor: Option<ParticipantId>) -> Self {
        let activity = ActivityEvent::new(ActivityType::SessionCreated, id.clone(), actor, None, ActivityResult::Success);
        Self { id, status: ProductionStatus::Created, participations: Vec::new(), recordings: Vec::new(), recording_coordination: None, activities: vec![activity] }
    }

    fn authorize(&self, actor: &ParticipantId, action: ProductionAction) -> Result<(), ProductionSessionError> {
        self.participations.iter().find(|participation| &participation.participant_id == actor).filter(|participation| participation.allows(action)).map(|_| ()).ok_or(ProductionSessionError::Unauthorized)
    }

    fn push_activity(&mut self, actor: Option<ParticipantId>, activity_type: ActivityType, target: Option<String>) {
        self.activities.push(ActivityEvent::new(activity_type, self.id.clone(), actor, target, ActivityResult::Success));
    }
    ...
    pub fn start_by(&mut self, actor: &ParticipantId) -> Result<(), ProductionSessionError> {
        ...
        if self.status != ProductionStatus::Created { return Err(ProductionSessionError::InvalidStateTransition); }
    ...
    pub fn complete_by(&mut self, actor: &ParticipantId) -> Result<(), ProductionSessionError> {
        ...
        if self.status != ProductionStatus::Active { return Err(ProductionSessionError::InvalidStateTransition); }
        if !self.has_owner() { return Err(ProductionSessionError::MissingOwner); }
    ...
    pub fn add_participation_by(&mut self, actor: &ParticipantId, participation: Participation) -> Result<(), ProductionSessionError> {
        if self.has_participant(&participation.participant_id) { return Err(ProductionSessionError::ParticipantAlreadyExists); }
        if self.participations.is_empty() {
            if !participation.is_owner() || actor != &participation.participant_id { return Err(ProductionSessionError::Unauthorized); }
        } else { self.authorize(actor, ProductionAction::ManageParticipants)?; }
        if self.status == ProductionStatus::Completed { return Err(ProductionSessionError::InvalidStateTransition); }
    ...
    pub fn begin_recording_by(&mut self, actor: &ParticipantId, recording_id: &RecordingId, participants: impl IntoIterator<Item = ParticipantId>) -> Result<(), ProductionSessionError> {
        self.authorize(actor, ProductionAction::ManageRecordings)?;
        if self.status != ProductionStatus::Active { return Err(ProductionSessionError::InvalidStateTransition); }
        if self.recording_coordination.is_some() { return Err(ProductionSessionError::RecordingCoordinationAlreadyActive); }
        let participants: Vec<_> = participants.into_iter().collect();
        for participant in &participants {
            self.participations.iter().find(|participation| &participation.participant_id == participant).filter(|participation| participation.allows(ProductionAction::ParticipateInRecording)).ok_or(ProductionSessionError::Unauthorized)?;
        }
        let mut coordination = RecordingCoordination::new(recording_id.clone(), participants).map_err(ProductionSessionError::RecordingCoordination)?;
    ...
    pub fn mark_recording_ready_by(&mut self, actor: &ParticipantId, recording_id: &RecordingId) -> Result<bool, ProductionSessionError> {
        ...
        if coordination.recording_id() != recording_id { return Err(ProductionSessionError::RecordingCoordinationNotFound); }
    ...
    pub fn start_recording_by(&mut self, actor: &ParticipantId, recording_id: &RecordingId) -> Result<(), ProductionSessionError> {
        self.authorize(actor, ProductionAction::ManageRecordings)?;
        if self.status != ProductionStatus::Active { return Err(ProductionSessionError::InvalidStateTransition); }
        let coordination = self.recording_coordination.as_ref().ok_or(ProductionSessionError::RecordingCoordinationNotFound)?;
        if coordination.recording_id() != recording_id || !coordination.is_ready() { return Err(ProductionSessionError::InvalidStateTransition); }
        let recording = self.recordings.iter_mut().find(|recording| recording.id() == recording_id).ok_or(ProductionSessionError::RecordingNotFound)?;
    ...
    pub fn has_participant(&self, participant_id: &ParticipantId) -> bool { self.participations.iter().any(|participation| &participation.participant_id == participant_id) }
    pub fn has_owner(&self) -> bool { self.participations.iter().any(|participation| participation.is_owner()) }
}
