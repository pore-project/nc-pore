...
class Element {
  constructor(tagName = 'div') { this.tagName = tagName.toUpperCase(); this.children = []; this.parentNode = null; this.attributes = new Map(); this.textContent = ''; this.className = '' }
  ...
  querySelector(selector) {
    return this._walk().find(node => matches(node, selector)) || null
  }
...
function makeExpect(actual) {
  const api = {
    ...
    toHaveBeenCalledWith(...expected) { assert.ok((actual?.mock?.calls || []).some(call => call.every((arg, i) => deepEqual(arg, expected[i])))) },
    ...
    rejects: { toThrow(expected) {
      return Promise.resolve(actual).then(() => { throw new Error('Expected promise to reject') }, error => {
...
if (failed) throw new Error(`${failed} JavaScript test(s) failed`)
