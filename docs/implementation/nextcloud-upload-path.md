...
`YYYY/MM/DD` and `HH-MM` are derived from the recording start timestamp. The display name is supplied by the host/session and sanitized for filesystem/WebDAV use. The stable ID guarantees uniqueness and remains independent of the display name.
